...
public class EachFantasmaNMA1 : MonoBehaviour
{
    // ********* FANTASMA AZUL (Nos persigue a intervalos de tiempo) *********
    private NavMeshAgent agent;
    private Vector3[] wayPoints;
    [SerializeField] private Transform pacman;

    public Vector3 initialPosition = new Vector3(-7.5f, 1.5f, 8.5f);
    [SerializeField] private Vector3 scaredPosition = new Vector3(-7.5f, 1.5f, 8.5f);
    private int indexWayPoints = 0;
    private float distanceToCurrentWayPoint = 1.5f;

    private bool followPacman;
    private float firstTimeNoFollow = 15.0f;
    private float timeNoFollow = 10.0f;
    private float timeFollow = 10.0f;
    ...
    void Start()
    {
        ...
        if (!followPacman)
        {
            Invoke("ChangeToFollow", firstTimeNoFollow);
        }
    }
    ...
    private void ChangeToFollow()
    {
        ...
        Invoke("ChangeToNoFollow", timeFollow);
    }

    private void ChangeToNoFollow()
    {
        followPacman = false;
        //ChangeGhostColor(Color.gray);
        print("NO FollowPacman!");

        Invoke("ChangeToFollow", timeNoFollow);
    }
